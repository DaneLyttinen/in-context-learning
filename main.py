...
def train_model(hidden_size):
    ...
    for num_tasks in num_tasks_range:
        train_results, test_results = init_dicts(train_results,test_results, hidden_size,num_tasks)
        ...
        train_loader,test_loader = init_datasets()
...
if __name__ == "__main__":
    hidden_size_range = [2**i for i in range(4, 10)]
    train_results_list = []
    test_results_list = []
    try:
        for hidden_size in hidden_size_range:
    ...
    filename = 'transformers_results_train.pkl'
    # Writing the dictionary to a file
    with open(filename, 'wb') as file:
        pickle.dump(all_train_results, file)

    filename = 'transformers_results_test.pkl'
    # Writing the dictionary to a file
    with open(filename, 'wb') as file:
